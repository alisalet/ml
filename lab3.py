...
for col in df.columns:
    ...
    if np.issubdtype(df_before[col].dtype, np.number):
        plt.hist(df_before[col].dropna(), bins=20, color='#7cb3fc', edgecolor='#68a6f7')
    else:
        counts=df_before[col].value_counts().sort_index()
        plt.bar(counts.index, counts.values, color='#7cb3fc', edgecolor='#68a6f7')
    ...
    if col in numeric_cols:
        plt.hist(encoded_df[col], bins=20, color='#a96df7', edgecolor='#a461fa')
    else:
        cat_cols_encoded=[c for c in encoded_df.columns if c.startswith(col.split()[0])]
        plt.bar(cat_cols_encoded, encoded_df[cat_cols_encoded].sum(), color='#a96df7', edgecolor='#a461fa')
